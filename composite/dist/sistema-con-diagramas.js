...
class Caja extends ElementoPedido {
     ...
     */
    generarConexionesMermaid(generador) {
...
class GeneradorMermaid {
    constructor() {
        ...
        this.mapaElementos = new Map();
    }
    /**
     * Obtiene o crea un ID único para un elemento
     */
    obtenerIdNodo(elemento) {
...
`;
     ...
     */
    generarPlantillaHTML(mermaid, titulo) {
        return `<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
...
class GestorPedidosConDiagramas {
    ...
    generarDiagramasPedido() {
        ...
        for (let i = 0; i < this.elementos.length; i++) {
            ...
            const diagrama = this.generador.generarDiagrama(elemento, `Elemento ${i + 1}: ${elemento.obtenerNombre()}`);
            // Guardar archivos
            this.generador.guardarDiagrama(diagrama, `elemento-${i + 1}-${nombre}`);
            this.generador.generarHTML(diagrama, `elemento-${i + 1}-${nombre}`, `Elemento ${i + 1}: ${elemento.obtenerNombre()}`);
        }
    ...
    generarDiagramaConsolidado() {
        ...
        const diagramaConsolidado = this.generador.generarDiagrama(cajaConsolidada, `Pedido Completo #${this.numeroPedido}`);
        this.generador.guardarDiagrama(diagramaConsolidado, `pedido-completo-${this.numeroPedido}`);
        this.generador.generarHTML(diagramaConsolidado, `pedido-completo-${this.numeroPedido}`, `Pedido Completo #${this.numeroPedido}`);
    }
}
// ============================================================================
